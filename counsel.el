...
`kmacro-counter-format-start', and `kmacro-counter-value-start'."
  (cons (propertize (concat "Last Macro: (" kmacro-counter-format-start ","
                            (number-to-string kmacro-counter-value-start) "): "
                            (format-kbd-macro (if (listp last-kbd-macro)
                                                  (nth 0 last-kbd-macro)
                                                last-kbd-macro)
                                              1))
                    ;; Depending on whether the last kmacro used a counter,
                    ;; it may or may not be a list.
                    'actual-kmacro (if (listp last-kbd-macro)
                                       last-kbd-macro
                                     (list last-kbd-macro
  ...
  (if (and (eq last-kbd-macro nil) (eq kmacro-ring nil))
    ...
    (ivy-read
     ...
     :action (lambda (selected-value)
               ...
               (let ((actual-kmacro (get-text-property 0 'actual-kmacro selected-value)))
                 (if actual-kmacro
                     (let ((kmacro-content (nth 0 actual-kmacro))
                           (kmacro-counter-format-start (nth 2 actual-kmacro)))
                       (kmacro-call-macro (or arg 1) t nil kmacro-content))
                   (progn
                     (counsel-kmacro-action-copy-counter selected-value)))))
...
`kmacro-ring' and set `last-kbd-macro' to that value."
  (let ((actual-macro (get-text-property 0 'actual-kmacro formatted-kmacro)))
...
(defun counsel-kmacro-action-copy-counter (chosen-string)
  "Set `kmacro-counter'  a value used by an existing macro."
  (let* ((actual-kmacro (get-text-property 0 'actual-kmacro chosen-string))
         (number (nth 1 actual-kmacro)))
    (kmacro-set-counter number)))

(defun counsel-kmacro-action-copy-format (chosen-string)
  "Copy the format of the chosen macro for the next defined macro."
  (let* ((actual-kmacro (get-text-property 0 'actual-kmacro chosen-string))
         (format (nth 2 actual-kmacro)))
    (kmacro-set-format format)))

(defun counsel-kmacro-action-execute-after-prompt (chosen-string)
  "Execute an existing keyboard macro, prompting for a starting counter value, a counter format, and the number of times to execute the macro.
...
If called with a prefix, will suggest those values."
  (let* ((default-string (if current-prefix-arg
                           ...
                           nil))
         (actual-kmacro (get-text-property 0 'actual-kmacro chosen-string))
         (kmacro-content (nth 0 actual-kmacro))
         ...
         (kmacro-counter-format-start (symbol-name (read-from-minibuffer ; This will return a symbol.
                                                    ...
                                                    nil nil t nil
                                                    kmacro-starting-format))))
    (kmacro-call-macro number-of-iterations t nil kmacro-content)))
