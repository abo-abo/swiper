            ...
            (if ivy--old-candidates
              ...
              (unless (ivy-set-index
                       (ivy--preselect-index
                        ...
                        ivy--all-candidates))
                (ivy--recompute-index ivy-text re ivy--all-candidates))))
          (setq ivy--old-candidates ivy--all-candidates)
           ...
           (if (and (boundp sym)
                    ...
                    (cond
                      ((and (consp sym-type)
                       ...
                       (setq candidates (delq nil (mapcar #'counsel--setq-doconst
                                                     (cdr sym-type)))))
                      ((eq sym-type 'boolean)
                       (setq candidates '(("nil" . nil) ("t" . t))))
                      (t nil)))
        ...
        (dolist (file (file-name-all-completions "" dir))
          (when (string-match suffix file)
            (unless (string-match "pkg.elc?$" file)
              ...
              (if (setq old-val (gethash short-name candidates))
                  (progn
                ...
                (puthash short-name
                         (cons (propertize
                               ...
                               dir)
                         candidates)))))))
    (maphash (lambda (_k v) (push (car v) res)) candidates)
    (nreverse res)))
...
The libraries are offered from `load-path'."
  (interactive)
  (let ((candidates (counsel-library-candidates)))
    (ivy-read "Load library: " candidates
              :action (lambda (x)
                        (load-library
...
The libraries are offered from `load-path'."
  (interactive)
  (let ((candidates (counsel-library-candidates)))
    (ivy-read "Find library: " candidates
...
(defun counsel--descbinds-candidates (&optional prefix buffer)
...
BUFFER defaults to the current one."
  (interactive)
  (ivy-read "Bindings: " (counsel--descbinds-candidates prefix buffer)
...
INITIAL-INPUT can be given as the initial minibuffer input."
  ...
  (let ((default-directory (counsel-locate-git-root)))
    (ivy-read "Find file: " (counsel-git-candidates)
              :initial-input initial-input
...
(defun counsel-git-occur (&optional _candidates)
...
(defun counsel-git-grep-occur (&optional _candidates)
...
(defun counsel-git-grep-query-replace ()
  ...
  (let* ((enable-recursive-minibuffers t)
    ...
    (ivy-exit-with-action
     (lambda (_)
       (let (done-buffers)
         (dolist (cand ivy--old-candidates)
...
(defun counsel-find-file-occur (&optional _candidates)
  (require 'find-dired)
  (cd ivy--directory)
...
(defun counsel-fzf-occur (&optional _candidates)
            ...
            (let* ((command-args (counsel--split-command-args ivy-text))
              ...
              (format cmd-template
                      (concat
                       switches
                       (shell-quote-argument regex))))))
         (candidates (counsel--split-string (shell-command-to-string cmd))))
    (swiper--occur-insert-lines (mapcar #'counsel--normalize-grep-match candidates))))

(defun counsel-ag-occur (&optional _candidates)
...
(defun counsel-grep-occur (&optional _candidates)
...
register tied to a mark in the message string."
  ...
  (let* ((all-markers ;;=> (register-character . marker)
          (append
...
(defun counsel-evil-marks ()
  ...
  (if (and (boundp 'evil-markers-alist)
      ...
      (let* ((counsel--mark-ring-calling-point (point)) ;; unwind to on cancel
...
(ivy-configure 'counsel-evil-marks
  :sort-fn #'ivy-string<)
          ...
          (cond ((eq display-style 'path)
                ...
                (t
                 ...
                 (setq name (counsel-outline--add-face
                             name level face-style custom-faces))))
          (push (cons name marker) candidates))
...
(defun counsel-wmctrl ()
  ...
  (interactive)
  (let* ((candidates1 (counsel--sl "wmctrl -l -p"))
         (candidates2 (delq nil (mapcar #'counsel--wmctrl-parse candidates1))))
    (ivy-read "window: " candidates2
...
This is determined by `counsel-compile-local-builds', which see."
  (let (candidates)
    (dolist (cmds counsel-compile-local-builds)
      ...
      (when cmds
        (push (if (listp cmds) cmds (list cmds)) candidates)))
    (apply #'append (nreverse candidates))))
...
(defun counsel-compile-env--format-hint (candidates)
  "Return a formatter for compile-env CANDIDATES."
  (let ((rmstr
...
(defun counsel-compilation-errors-candidates ()
  (cl-loop
...
(defun counsel-compilation-errors ()
  "Compilation errors."
  (interactive)
  (ivy-read "compilation errors: " (counsel-compilation-errors-candidates)
            :require-match t
