...
To highlight a section without introducing new subheading, use
definition lists. The definition list "owns" subsequent text if the
text is indented by 5 spaces. Use ~C-q~ to indent paragraphs. Start
new paragraphs with 5 spaces indented. To separate definition lists
from regular lists, use two newlines.
...
oldest version that runs Ivy with faces display.
...
- User Option =ivy-wrap= ::
    Specifies the wrap-around behavior for ~C-n~ and ~C-p~. When this
...
- User Option =ivy-height= ::
    ...
    =ivy-height= by default is 10 lines.
...
- ~TAB~ (=ivy-partial-or-done=) ::
     Attempts partial completion; extends current input as much as
...
- ~C-M-j~ (=ivy-immediate-done=) ::
     ...
     This is useful for creating a new file after calling =find-file=.
     Otherwse ~C-j~ will pick whatever file happens to match /the
     current input/.
...
Ivy's completion functions rely on a regex builder. It builds a valid
regex syntax from the input string. Regex matches are then displayed
as candidates. Each collection can be assigned its own regex builder
by customizing =ivy-re-builders-alist=.

The =ivy-re-builders-alist= keys are collection names, and the values
...
Where =read-file-name-internal= is a function that is passed as the
...
2. =collection= argument passed to =ivy-read= is a function and has an
   entry in the =ivy-re-builders-alist=.
...
rebuilding it into a valid regex.
...
- =ivy-confirm-face= ::
     ...
     prompt will be appended to "(confirm)". Press ~RET~ once more
     to confirm, or any key to continue with completion.
...
- User Option =ivy-on-del-error-function= ::
     Specifies action for ~DEL~ (=ivy-backward-delete-char=).
...
:END:
An action is a function that is called during completion. This
function takes a single string argument, which is the selected
candidate.

- Window context when calling an action ::
     The minibuffer window is the context. =insert= text will insert
     text into the minibuffer.

     To change the context from the minibuffer to the window where the
     completion began, use the =with-ivy-window= wrapper macro.
...
:END:
- ~C-m~ (=ivy-done=) calls the current or default action.
...
:END:
Append custom actions to the default list of actions. This can be done
either for one command at a time, or for all commands at once. Each
command typically has one default action.

Assign single letters for selecting a command (by convention). ~o~ is
reserved for the default command. ~M-o o~ is equivalent to ~C-m~.
...
~M-o y~ invokes =ivy-yank-action=, and
...
The number 1 above is the desired index of the default action. Each
...
:END:
To examine each action with each candidate:
...
- =projectile= ::
     ...
     #+end_src
- =helm-make= ::
     Helm requires this seeting for ivy completion.
...
- User Option =ivy-extra-directories= ::
     Optton to remove =../= and =./= during file name completion.
...
- User Option =ivy-use-virtual-buffers= ::
     ...
     =ivy-switch-buffer= for completion candidates.
     ...
     =ivy-virtual= markers, will open the corresponding file. This
     completion is when using =ivy-switch-buffer=.
...
- =preselect= ::
     ...
     When empty, select the candidate matching =preselect=.
- =initial-input= ::
     Use the =preselect= argument instead of this. This string
     argument is included for compatibility with =completing-read=,
     which inserts into the minibuffer.
- =history= ::
     Name of symbol to store history. See =completing-read=.
- =keymap= ::
     A keymap composed with =ivy-minibuffer-map= and has priority over
     it. =keymap= can be modified later at any later stage. 
- =update-fn= ::
...
- =matcher= ::
     ...
     usage.
- =dynamic-collection= ::
     When non-nil, the =collection= is dynamically re-generated after
     each input change. See =counsel-locate= for an example usage.
- =caller= ::
...
The =action= argument may be omitted to make =ivy-read= return the
sring result for subsequent custom use, if any. But for normal use we
specify the =action=.
...
Here are the arguments of the above function explained:
...
- The =sort= argument is set to =t= so choosing between similar
  candidates becomes easier. However on Emacs systems with many
  packages loaded, the collection size could exceed the default value
  as set by =ivy-sort-max-size= (30000). Adjust this variable to
  choose between sorting time and completion start-up time.
- The =action= argument calls =describe-function= on the interned
  selected candidate.
- The =caller= argument uniquely identifies this completion session.
  Identifying is necessary for customizations.
...
- =counsel-locate-function= takes a string argument and returns a list
  of strings. It is incompatible =all-completions=.
- =counsel-more-chars= is a simple function that asks the user for
  more input with prompts, such as =("2chars more")=.
- =counsel--async-command= is an API simplification that takes a
  single string argument suitable for =shell-command-to-string=. It is
  useful for prototyping the function as a non-asynchronous command
  using =shell-command-to-string= and =split-string= to produce a
  collection. Then to make it async, simply swap it with
  =counsel--async-command=.
...
- =action= argument uses =with-ivy-window= wrapper, since we want to open the
  ...
  called.
- =unwind= argument is set to =#'counsel-delete-process=: so ~C-g~
  kills the running process created by =counsel--async-command=.
- The =caller= argument uniquely identifies this completion session.
  Identifying is necessary for customizations
